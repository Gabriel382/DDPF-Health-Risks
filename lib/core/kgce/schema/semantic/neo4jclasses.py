...
class Neo4jNode:
    def __init__(self,name, class_name="", dict_param={}):
        self.name = name
        self.class_name = class_name
        self.dict_param = dict_param
        
    def _neostring(self, class_name=None, dict_param=None):
        if class_name is not None:
            self.class_name = class_name
        if dict_param is not None:
            self.dict_param = dict_param
        return ":" + self.class_name + " { " + ", ".join([str(x) + ": " + (str(float(y)) if type(y)==str and y.replace('.','',1).replace(',','',1).isdigit() else '"' + str(y).replace("'","").replace('"','') + 
        '"' ) for x,y in list(self.dict_param.items())]) + "}"
...
class Neo4jRelation:
    ...
    def _neostring(self, class_name=None, dict_param=None):
        ...
        if dict_param is not None:
            self.dict_param = dict_param
        return ":" + self.class_name + " { " +  ", ".join([str(x) + ": " + (str(float(y)) if type(y)==str and y.replace('.','',1).replace(',','',1).isdigit() else '"' + y + 
        '"') for x,y in list( self.dict_param.items())]) + " }"
